...
router.get('/', isLoggedIn, async (req, res) => {
    if(req.user.cedula === '123') {
        ...
        res.render('adm/bonos', { dataUsuarios, dataBonos})
    } else {
        ...
        res.render('bonos/lista', { data })
    }
})

router.get('/nuevo/:id', isLoggedIn, async(req, res)=>{
    if(req.user.cedula === '123'){
        const {id} = req.params
        const data = await pool.query(`
        Select a.id, a.cedula, a.nombre, a.p_apellido, a.s_apellido, substr(a.fecha_contrato, 1, 10) as fecha_contrato, b.nombre_cargo
        ...
        `, [id])
        console.log(data)
        res.render('adm/nuevoBono', {data: data[0]})
    } else {
        res.send('No deberia estar aqui')
    }
})

router.post('/nuevo/:id', isLoggedIn, async (req, res)=>{
    if(req.user.cedula === '123'){
        const {id} = req.params
